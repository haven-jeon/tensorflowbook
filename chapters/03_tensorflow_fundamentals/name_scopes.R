...
with(tf$name_scope("Scope_A"),{
  a <- tf$add(1L, 2L, name="A_add")
  b <- tf$mul(a, 3L, name="A_mul")
})


with(tf$name_scope("Scope_B"), {
  c <- tf$add(4L, 5L, name="B_add")
  d <- tf$mul(c, 6L, name="B_mul")
})
...
with(graph$as_default(),{
  in_1 <- tf$placeholder(tf$float32, shape=list(), name="input_a")
  in_2 <- tf$placeholder(tf$float32, shape=list(), name="input_b")
  const <- tf$constant(3L, dtype=tf$float32, name="static_value")
